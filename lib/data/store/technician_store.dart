...
class TechnicianStore extends ChangeNotifier {
  ...
  final List<TechnicianRequestModel> _requests = [
    const TechnicianRequestModel(
      ...
      vehicleYear: '2018',
      issueTitle: 'صوت غريب عند بدء الإشعال',
      issueDescription:
    ...
    const TechnicianRequestModel(
      ...
      vehicleYear: '2019',
      issueTitle: 'سخونة أعلى من الطبيعي',
      issueDescription:
    ...
    const TechnicianRequestModel(
      ...
      vehicleYear: '2020',
      issueTitle: 'اهتزاز أثناء الفرملة',
      issueDescription:
    ...
    const TechnicianRequestModel(
      ...
      riskLabel: 'منخفضة',
      status: TechnicianRequestStatus.inProgress,
      minCost: 800,
      maxCost: 1100,
    ...
    const TechnicianRequestModel(
      ...
      createdAt: '12 أغسطس',
      riskLabel: 'متوسطة',
      status: TechnicianRequestStatus.completed,
      minCost: 1500,
      maxCost: 1800,
  ...
  final conversations = const [
    TechnicianConversationModel(
      ...
      customerName: 'كريم خالد',
      subtitle: 'متصل الآن',
      lastMessage: 'تمام، ابدأ العمل الآن',
      time: '12:37',
      requestId: 'job-1',
    ),
    TechnicianConversationModel(
      id: 'c2',
      customerName: 'سيف عماد',
      subtitle: 'آخر ظهور أمس',
      lastMessage: 'شكرًا يا هندسة',
      time: 'أمس',
      requestId: 'job-2',
    ),
  ];

  List<TechnicianRequestModel> get requests => List.unmodifiable(
    _requests.where(
      (e) =>
          e.status == TechnicianRequestStatus.newRequest ||
          e.status == TechnicianRequestStatus.offerSent,
    ),
  );
  List<TechnicianRequestModel> get currentJobs => List.unmodifiable(
    _requests.where(
      (e) =>
          e.status == TechnicianRequestStatus.accepted ||
          e.status == TechnicianRequestStatus.inProgress,
    ),
  );
  List<TechnicianRequestModel> get previousJobs => List.unmodifiable(
    _requests.where((e) => e.status == TechnicianRequestStatus.completed),
  );
  TechnicianRequestModel byId(String id) =>
      _requests.firstWhere((e) => e.id == id);

  TechnicianAppBarModel get appBar => TechnicianAppBarModel(
    technician: profile,
    newRequests: requests
        .where((e) => e.status == TechnicianRequestStatus.newRequest)
        .length,
    pendingOffers: requests
        .where((e) => e.status == TechnicianRequestStatus.offerSent)
        .length,
  ...
  void reject(String id) {
    ...
    _requests[i] = _requests[i].copyWith(
      status: TechnicianRequestStatus.rejected,
    );
    notifyListeners();
  }
  ...
  void sendOffer(
    ...
    if (i < 0) return;
    _requests[i] = _requests[i].copyWith(
      status: TechnicianRequestStatus.offerSent,
